...
import { client, initializeDb, seedAdminUser } from './db.js';
...
app.get('/products', async (req, res) => {
    try {
        const result = await client.execute({
            sql: `SELECT * FROM products WHERE is_published = 1 AND publication_end_date > CURRENT_TIMESTAMP`,
            args: []
        });
...
app.post('/login', async (req, res) => {
    ...
    try {
        // 🟢 ACTUALIZADO: Se añade premium_expires_at a la consulta
        const result = await client.execute({
        ...
        // 🟢 LÓGICA NUEVA: Verificar si el premium ha expirado
        const currentRole = await checkAndExpirePremium(user);
        ...
        res.status(200).json({
            ...
            user: {
                ...
                username: user.username,
                email: user.email,
                role: currentRole // Devuelve el rol actualizado (Usuario o Distribuidor Premium)
            }
        });
...
app.post('/register', async (req, res) => {
    ...
    const SPECIAL_REFERRAL_CODE = 'BLD231';
    ...
    if (referralCodeUsed && referralCodeUsed.toUpperCase() === SPECIAL_REFERRAL_CODE) {
        defaultRole = 'Pending';
        defaultIsApproved = 0;
    }
    ...
    try {
        ...
        if (defaultIsApproved === 0) {
            io.emit('pendingUsersUpdated');
            return res.status(201).json({
                message: 'Solicitud de Distribuidor registrada exitosamente. Esperando aprobación del administrador.',
                user: {
                    id: newUser.id,
...
app.post('/admin/user/approve', async (req, res) => {
    ...
    try {
        // 1. Generar código de referido único (Intenta hasta 10 veces si hay colisión)
        let newReferralCode;
        let isUnique = false;
        let attempts = 0;
        ...
        // 2. Actualizar usuario: Aprobar, asignar Rol distribuidor y guardar su nuevo Código
        await client.execute({
            sql: 'UPDATE users SET is_approved = 1, role = ?, referral_code = ? WHERE id = ?',
            args: ['distribuidor', newReferralCode, numericUserId]
        });

        // Actualizar tablas de admin
        io.emit('pendingUsersUpdated');
        io.emit('usersUpdated');

        // 🔥 3. Notificar al usuario con su nuevo código para la fiesta
        const targetSocketId = userSocketMap.get(numericUserId);
        if (targetSocketId) {
            io.to(targetSocketId).emit('applicationResult', {
                status: 'approved',
                referralCode: newReferralCode // Enviamos el código generado
            });
        }

        res.status(200).json({ message: 'Usuario aprobado y código generado: ' + newReferralCode });
    } catch (error) {
...
app.post('/supplier/refund', async (req, res) => {
    ...
    try {
        ...
        const stockRes = await client.execute({ sql: `SELECT sold_to_user_id, product_id FROM product_stock WHERE id = ?`, args: [stockId] });
        if (stockRes.rows.length === 0) return res.status(404).json({ message: 'No encontrado.' });
        const { sold_to_user_id: buyerId, product_id: productId } = stockRes.rows[0];
        ...
        // 3. Devolver dinero al usuario
        const userRes = await client.execute({ sql: 'SELECT balance, transactions_history FROM users WHERE id = ?', args: [buyerId] });
        const user = userRes.rows[0];
        const newBalance = user.balance + parseFloat(amountToRefund);
        ...
        history.unshift({
            ...
            type: 'credit',
            status: 'Devuelto' // 🟢 ESTADO SOLICITADO
        });

        await client.execute({ sql: 'UPDATE users SET balance = ?, transactions_history = ? WHERE id = ?', args: [newBalance, JSON.stringify(history), buyerId] });

        // 4. Restaurar Stock (Liberar)
        await client.execute({
...
app.post('/admin/user/update', async (req, res) => {
    ...
    try {
        ...
        if (roleToUpdate) {
            if (roleToUpdate !== 'Usuario' && roleToUpdate !== 'Premium' && roleToUpdate !== 'Admin' && roleToUpdate !== 'distribuidor' && roleToUpdate !== 'proveedor') {
    ...
    try {
        ...
        if (referrerId && isFirstRecharge) {
            ...
            if (referrerResult.rows.length > 0) {
                ...
                // B. Determinar Porcentaje (10% Premium, 5% Estándar)
                // 🟢 LÓGICA MODIFICADA AQUÍ
                const isPremiumReferrer = referrer.role === 'Distribuidor Premium';
                const commissionRate = isPremiumReferrer ? 0.10 : 0.05;
...
app.post('/user/purchase', async (req, res) => {
    const { userId, productName, amount, platform, type, provider, duration, terms, quantity = 1, delivery } = req.body;
    const numericUserId = parseInt(userId);
    const numericQty = parseInt(quantity);

    if (!numericUserId || !productName) return res.status(400).json({ message: 'Datos incompletos.' });

    try {
        // 1. Validar Usuario y Saldo
        const userResult = await client.execute({ sql: 'SELECT balance, transactions_history FROM users WHERE id = ?', args: [numericUserId] });
        if (userResult.rows.length === 0) return res.status(404).json({ message: 'Usuario no encontrado.' });
        const user = userResult.rows[0];
        if (user.balance < parseFloat(amount)) return res.status(400).json({ message: 'Saldo insuficiente.' });

        // 2. Buscar Producto
        const productResult = await client.execute({
            sql: 'SELECT id, stock, duration, delivery, creator_user_id, product_details, instructions, is_renewable, price_renewal_standard, price_renewal_premium, price_standard, price_premium FROM products WHERE name = ? AND platform = ? LIMIT 1',
        ...
        // 🟢 CAMBIO CRÍTICO: Eliminamos el "if (!isOrderRequest)".
        // AHORA TODOS LOS PRODUCTOS CONSUMEN STOCK, INCLUSO LOS "A PEDIDO".
        
        // Traemos TODO el stock disponible
        const allStock = await client.execute({
            sql: "SELECT id, data FROM product_stock WHERE product_id = ? AND is_sold = 0",
            args: [productId]
        });
        ...
        for (const row of allStock.rows) {
            ...
            if (availableInRow > needed) {
                // Split (Resta y crea copia vendida)
                ...
                for (let k = 0; k < needed; k++) {
                    const soldCopy = { ...data, quantity: 1 };
                    const insertRes = await client.execute({ sql: "INSERT INTO product_stock (product_id, provider_user_id, data, is_sold, sold_to_user_id, sold_at) VALUES (?, ?, ?, 1, ?, CURRENT_TIMESTAMP)", args: [productId, product.creator_user_id, JSON.stringify(soldCopy), numericUserId] });
                    credentialsList.push({ stockId: insertRes.lastInsertRowid.toString(), productId, ...soldCopy });
                }
                collectedCount += needed;
            } else {
                // Consume total
                updatesToExecute.push(client.execute({ sql: "UPDATE product_stock SET is_sold = 1, sold_to_user_id = ?, sold_at = CURRENT_TIMESTAMP WHERE id = ?", args: [numericUserId, row.id] }));
                credentialsList.push({ stockId: row.id, productId, ...data });
                collectedCount += availableInRow;
            }
        }

        if (collectedCount < numericQty) {
            return res.status(409).json({ message: `Stock insuficiente. Disponibles: ${collectedCount}` });
        }
        await Promise.all(updatesToExecute);
        
        // Actualizar contador global
        // Nota: Para consistencia, usamos la lógica de resta directa aquí también
        await client.execute({ sql: 'UPDATE products SET stock = stock - ? WHERE id = ?', args: [numericQty, productId] });

        // ... (Resto del código igual: obtener proveedor, crear transacción, etc.) ...
        
        const providerResult = await client.execute({ sql: 'SELECT phone, username FROM users WHERE id = ?', args: [product.creator_user_id] });
        ...
        const transactionId = `#ORD-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
        ...
        const newTransaction = {
            ...
            status: 'Completada',
            details: {
                productName, platform, planType: type,
                provider: providerData ? providerData.username : provider,
                duration: product.duration, terms, cost: parseFloat(amount),
                fullCredentials: credentialsList,
                purchaseDate: purchaseDate.toLocaleDateString('es-PE'),
        ...
        // Si es "A pedido", TAMBIÉN guardamos en orders para que el proveedor sepa
        if (isOrderRequest) {
            await client.execute({
                sql: `INSERT INTO orders (purchase_id, buyer_user_id, provider_user_id, product_id, quantity, total_price, status) VALUES (?, ?, ?, ?, ?, ?, 'Pending')`,
                args: [transactionId, numericUserId, product.creator_user_id, productId, numericQty, amount]
            });
        ...
        await client.execute({
            sql: 'UPDATE users SET balance = ?, transactions_history = ? WHERE id = ?',
            args: [newBalance, JSON.stringify([newTransaction, ...currentHistory]), numericUserId]
        });

        io.emit('transactionsUpdated');
        io.emit('productsUpdated'); // Siempre actualizamos stock
        ...
        if (targetSocketId) {
            io.to(targetSocketId).emit('transactionApproved', {
                ...
                amount: parseFloat(amount),
                message: `¡Tu compra de ${productName} ha sido completada!`,
            });
        }
        ...
        res.status(200).json({
            ...
            purchaseDate: newTransaction.details.purchaseDate,
            expirationDate: newTransaction.details.expirationDate,
        });
...
app.get('/supplier/sales/:providerId', async (req, res) => {
    ...
    try {
        const result = await client.execute({
            sql: `
                SELECT 
                    p.id as product_id,       -- 🟢 CRÍTICO: Necesario para la devolución
                    u.id as buyer_id,         -- 🟢 CRÍTICO: Necesario para la devolución
        ...
        const sales = result.rows.map(row => {
            ...
            return {
                productId: row.product_id, // 🟢 Mapeado
                buyerId: row.buyer_id,     // 🟢 Mapeado
                ...
                unitPrice: unitPrice, // Guardamos el precio unitario para facilitar el cálculo
                // Usamos sold_at tal cual viene de la BD para la búsqueda exacta
...
app.post('/user/purchase/renovate', async (req, res) => {
    ...
    if (!numericUserId || !purchaseId || isNaN(numericMonths) || numericMonths < 1 || isNaN(numericFinalPrice)) {
        return res.status(400).json({ message: 'Datos de renovación incompletos o inválidos.' });
    }

    try {
        // 1. Obtener usuario y su historial
        const userResult = await client.execute({
            sql: 'SELECT balance, transactions_history, role FROM users WHERE id = ?',
            args: [numericUserId]
        });
        ...
        if (user.balance < numericFinalPrice) {
            return res.status(400).json({ message: 'Saldo insuficiente para la renovación.' });
        }

        let history = JSON.parse(user.transactions_history || '[]');
        let transactionFound = false;

        // 2. Encontrar la transacción original y calcular la nueva fecha de expiración
        const updatedHistory = history.map(tx => {
            if (tx.id === purchaseId && tx.type === 'debit') {
                transactionFound = true;
                
                let startRenewalDate;
                const now = new Date();
                
                // Usamos la fecha de expiración actual como base para la renovación
                if (tx.details.expirationDate) {
                    const existingExpiry = new Date(tx.details.expirationDate);
                    // Si ya expiró, la renovación comienza hoy
                    startRenewalDate = existingExpiry > now ? existingExpiry : now;
                } else {
                    // Si no tiene fecha de expiración, asumimos que empieza hoy
                    startRenewalDate = now;
                }

                // Clonar y sumar los meses
                let newExpiry = new Date(startRenewalDate.getTime());
                newExpiry.setMonth(newExpiry.getMonth() + numericMonths);
                
                // Formatear a dd/mm/yyyy
                const newExpiryDateStr = newExpiry.toLocaleDateString('es-PE');

                // 3. Crear el nuevo registro de débito por renovación
                const renovationTx = {
                    id: Date.now(), // Nuevo ID de transacción de débito (timestamp)
                    date: new Date().toLocaleDateString('es-PE'),
                    description: `Renovación de ${tx.details.productName} por ${numericMonths} meses (${planType})`,
                    ...
                    details: {
                        ...tx.details,
                        planType: planType,
                        cost: numericFinalPrice,
                        // 🟢 Actualizar las fechas de la transacción original y la nueva transacción de renovación
                        purchaseDate: new Date().toLocaleDateString('es-PE'),
                        expirationDate: newExpiryDateStr,
                        isRenovation: true
                    }
                };
                
                // 4. Actualizar la transacción original (solo la fecha de expiración)
                tx.details.expirationDate = newExpiryDateStr;
                
                return [tx, renovationTx]; // Devolver la original actualizada y la nueva de débito
            }
            return tx;
        }).flat(); // Aplanar el array para insertar el nuevo débito

        if (!transactionFound) return res.status(404).json({ message: 'Transacción original de compra no encontrada.' });

        // 5. Actualizar saldo y guardar historial
        const newBalance = user.balance - numericFinalPrice;

        await client.execute({
            sql: 'UPDATE users SET balance = ?, transactions_history = ? WHERE id = ?',
            args: [newBalance, JSON.stringify(updatedHistory), numericUserId]
        });
        ...
        res.status(200).json({ 
            ...
            newBalance: newBalance.toFixed(2),
            newExpirationDate: updatedHistory.find(tx => tx.id === purchaseId)?.details?.expirationDate
        });

    } catch (error) {
        console.error('Renovation error:', error);
        res.status(500).json({ message: 'Error interno al procesar la renovación.', details: error.message });
    }
});
...
app.post('/supplier/withdraw', async (req, res) => {
    ...
    try {
        ...
        if (currentBalance <= 0) {
            return res.status(400).json({ message: "No tienes saldo para retirar." });
        }

        // C. Calcular montos (10% comisión)
        const fee = currentBalance * 0.10;
        const finalAmount = currentBalance - fee;
        ...
        await client.execute({
            ...
            args: [numericUserId, currentBalance, finalAmount]
        });

        // E. 🔥 DESCONTAR SALDO INMEDIATAMENTE (VACIAR CARTERA)
        await client.execute({
        ...
        io.emit('withdrawalsUpdated'); // Actualiza tabla Admin y Proveedor
        io.emit('usersUpdated');       // 🔥 Actualiza el Header con $0.00 inmediatamente
...
app.post('/admin/withdraw/manage', async (req, res) => {
    ...
    try {
        ...
        if (action === 'approve') {
            // ✅ APROBAR: 
            // El dinero YA SE QUITÓ cuando el usuario hizo la solicitud (saldo está en 0 o lo que quedaba).
            // Solo cambiamos el estado a 'Approved' para confirmar que el pago externo se hizo.
            await client.execute({
            ...
            // ❌ RECHAZAR: 
            // El dinero se había quitado temporalmente. DEBEMOS DEVOLVERLO.
            ...
            const targetSocketId = userSocketMap.get(withdrawal.user_id);
            if (targetSocketId) {
                io.to(targetSocketId).emit('transactionApproved', { // Reusamos este evento para notificaciones toast
        ...
        io.emit('withdrawalsUpdated'); // Actualiza la tabla del Admin y del Proveedor (Estado)
        io.emit('usersUpdated');       // Actualiza el Header del Proveedor (Saldo devuelto si aplica)
